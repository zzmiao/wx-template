...
Page({
  ...
  uploadScaleStart: function(e) {
    ...
    ctx.setGlobalAlpha(this.data.transparent); //设置半透明,设置全局画笔透明度
    let currentPoint = {
      x: e.touches[0].x,
      y: e.touches[0].y
    }
    let currentLine = this.data.currentLine;
    currentLine.unshift({ //unshift() 方法可向数组的开头添加一个或更多元素，并返回新的长度
  ...
  //保存到相册
  saveCanvasAsImg() {
    //canvasToTempFilePath:把当前画布指定区域的内容导出生成指定大小的图片
